...
describe("Test de RUTAS", ()=>{
    ...
    describe("GET /rickandmorty/login", ()=>{
        it("Responde con true para las credenciales correctas",async ()=>{
            const {body} = await agent.get('/rickandmorty/login?email=francisco@rickandmorty.com&password=1234569')
            expect(body).toEqual({
                access:true
            });
        });
        it("Responde con false para las credenciales correctas",async ()=>{
            const {body} = await agent.get('/rickandmorty/login?email=francisco@rickandmorty.com&password=1234')
    ...
    describe( "DELETE /rickandmorty/fav/:id", ()=>{
        it("Debe devolver el arreglo original si no encuentra ningun personaje con el ID enviado", async()=>{
        ...
        it('Debe eliminar el personaje indicado', async()=>{
